...
int main() {
   ...
   bool32 button_down  = 0;
   bool32 button_up    = 0;
   bool32 button_right = 0;
   bool32 button_left  = 0;
         ...
         switch(_event->response_type &~0x80) {
            ...
            case XCB_KEY_PRESS:
            {
               ...
               if((_event->response_type &~0x80) == XCB_KEY_PRESS && !keys_down[keycode]) {
                  keys_down[keycode] = 1;
                  if(keycode == 25) {
                     keyboard_input_process(&new_controller->up,    TRUE);
                  } else if(keycode == 39) {
                     keyboard_input_process(&new_controller->down,  TRUE);
                  } else if(keycode == 38) {
                     keyboard_input_process(&new_controller->left,  TRUE);
                  } else if(keycode == 40) {
                     keyboard_input_process(&new_controller->right, TRUE);
                  }
