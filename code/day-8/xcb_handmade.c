...
alsa_init(int samples_per_second, int samples_per_write) {
   ...
   snd_pcm_hw_params_set_buffer_size(_pcm, _pcm_hw_params, samples_per_second*4);
...
int main() {
   ...
   //Note(LAG): We devide the samples_per_second per the amount of writes we want to make to the buffer per second
   //Note(LAG): samples_per_second/1 = 1write/s, samples_per_second/2 = 2writes/s ...
   int samples_per_write = samples_per_second/1;
   ...
   while (is_running) {
      ...
      if(avail > samples_per_write) {
         ...
         int result;
         while((result = snd_pcm_writei(_pcm, samples_buffer, samples_per_write)) != samples_per_write) {
            if(result == -EPIPE) {
